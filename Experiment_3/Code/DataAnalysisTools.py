...
def DataIntersectionPoint(data_set1, data_set2):
    """
    Calculates the point(s) of intersection between the two data sets.
    Uses an inverse exponential data fit.
    """
    x1, x1_unc, y1, y1_unc = data_set1
    x2, x2_unc, y2, y2_unc = data_set2

    def fit_func(x, a, b, c):
        return a * np.exp(b * x) + c

    fit1_coeffs, _ = curve_fit(fit_func, x1, y1)
    func1 = lambda x: fit_func(x, *fit1_coeffs)
    fit2_coeffs, _ = curve_fit(fit_func, x2, y2)
    func2 = lambda x: fit_func(x, *fit2_coeffs)

    difference_func = lambda x: func1(x) - func2(x)

    x_intersection = root_scalar(difference_func, bracket=(min(x1), max(x1))).root
    ...
    # fit1_coeffs = np.polynomial.polynomial.Polynomial.fit(x1, y1, 10)
    # poly1 = np.polynomial.polynomial.Polynomial(fit1_coeffs)
    # fit2_coeffs = np.polynomial.polynomial.Polynomial.fit(x2, y2, 10)
    # poly2 = np.polynomial.polynomial.Polynomial(fit2_coeffs)
    #
    # difference_poly = poly1 - poly2
    #
    # x_intersections = difference_poly.roots()
    #
    # y_intersections = poly1(x_intersections)
    #
    # print("Intersection points:")
